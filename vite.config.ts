...
// Note: the generated template also ships a plugin that strips the `crossorigin` attribute from
// the built HTML. It is deliberately absent here: widgets run in sandboxed `about:srcdoc` iframes,
// where Chrome needs `crossorigin` to load the widget's own JS and CSS.
